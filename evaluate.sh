...
EC_path="/MSGscore" # put your project files path here
base_videos_dir="/videos/outputs_Alien_in_Redplanet" # put your video files path here
prompt_dir="./videos/inference_videos/prompt.csv" # put your prompt file path here
# prompt should be written in one prompt in one line
...
# EvalCrafter
find "$base_videos_dir" -type d -links 1 | while read -r dir_videos; do
    ...
    if [ -d "$EC_path/$results_dir" ]; then
        new_results="${EC_path}/EvalCrafter/${results_dir}_${timestamp}_${folder_name}"
...
find "$base_videos_dir" -type d -links 1 | while read -r folder; do
